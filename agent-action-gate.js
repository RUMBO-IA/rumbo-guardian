(function(root,factory){
  ...
  function assessAgentAction(rawAction={},options={}){
    ...
    if(!action.effect||!EFFECTS.has(action.effect)) deny('unknown_effect','Unknown or missing action effect; fail closed.');
    ...
    if(EXTERNAL_EFFECTS.has(action.effect)){
      ...
      if(!action.authorization.authorizationId) review('missing_authorization_id','Bound authorization has no stable identifier.','authorization_binding');
      else if(action.previouslyExecutedAuthorizationIds.includes(action.authorization.authorizationId)) deny('authorization_replay_detected','The bound authorization identifier was already executed.');

      if(!action.authorization.actionDigest) review('missing_action_digest','Authorization is not bound to an action digest.','authorization_binding');
      else if(!digest) review('digest_verification_unavailable','Action digest cannot be independently recomputed in this runtime.','authorization_binding');
      else if(action.authorization.actionDigest!==digest) deny('authorization_action_mismatch','Authorization digest does not match the proposed action.');

      if(expires===null) review('missing_or_invalid_authorization_expiry','Bound authorization expiry cannot be validated.','authorization_binding');
      else {
    ...
    if(EXTERNAL_EFFECTS.has(action.effect)&&!action.target) review('missing_target','External-effect action has no explicit target.','target');
    if(EXTERNAL_EFFECTS.has(action.effect)&&!action.targetVerified) review('unverified_target','External-effect action target has not been verified.','target_verification');
    if(HIGH_RISK_EFFECTS.has(action.effect)&&action.evidenceCount<1) review('insufficient_evidence','High-risk action has no supporting evidence.','evidence');
    if(DESTRUCTIVE_EFFECTS.has(action.effect)&&!action.reversible&&!action.freshConfirmation) review('irreversible_without_fresh_confirmation','Irreversible or destructive action needs fresh confirmation.','fresh_confirmation');
    if(action.handlesSecrets&&!action.destinationTrusted) deny('secret_to_untrusted_destination','Secrets may not be sent to an untrusted destination.');
    ...
    if(action.effect==='purchase'){
      ...
      if(action.amount!==null&&action.spendLimit!==null&&action.amount>action.spendLimit) deny('spend_limit_exceeded',`Purchase amount ${action.amount} exceeds spend limit ${action.spendLimit}.`);
    }
  ...
  function assessPlan(actions=[],options={}){
    ...
    return {decision,assessments,requiredGates:uniq(assessments.flatMap(x=>x.requiredGates)),policyVersion:'RUMBO_AGENT_ACTION_GATE_V2_1_PARAMETERS_BOUND',failClosed:true};
  }
