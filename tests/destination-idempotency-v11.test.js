...
async function main(){
  ...
  try{
    ...
    }
    {
      ...
      const {prepared,idempotencyKey}=reserveDestination(store,input,'v11-reserved-recovery');
    ...
    }
    {
      ...
      assert.equal(initial.effectOutcome,'FAILED_OR_UNKNOWN');assert.equal(store.getExecution('v11-reconcile').state,'FAILED_OR_UNKNOWN');assert.equal(store.getDestination('v11-reconcile').state,'PENDING');
      const reconciled=await d.reconcileUnknown({authorizationId:'v11-reconcile'});
    ...
    }
    {
      ...
      assert.equal(initial.effectOutcome,'FAILED_OR_UNKNOWN');assert.equal(store.getExecution('v11-pending').state,'FAILED_OR_UNKNOWN');
      const reconciled=await d.reconcileUnknown({authorizationId:'v11-pending'});assert.equal(reconciled.stage,'reconciliation_pending');assert.equal(store.getExecution('v11-pending').state,'FAILED_OR_UNKNOWN');assert.equal(store.getDestination('v11-pending').state,'PENDING');store.close();ok();
    }
    {
      ...
      assert.equal(result.effectOutcome,'FAILED_OR_UNKNOWN');assert.equal(result.evidence.reason,'destination_idempotency_key_mismatch');assert.equal(store.getExecution('v11-bad-evidence').state,'FAILED_OR_UNKNOWN');assert.equal(store.getDestination('v11-bad-evidence').state,'PENDING');
      const reconciled=await d.reconcileUnknown({authorizationId:'v11-bad-evidence'});assert.equal(reconciled.effectOutcome,'SUCCEEDED');assert.equal(reconcileCalls,1);assert.equal(store.getExecution('v11-bad-evidence').state,'SUCCEEDED');store.close();ok();
    }
    {
      const store=new SQLiteExecutionStoreV11(path.join(tmp,'live-started.sqlite')),input={message:'live'};let reconcileCalls=0;
