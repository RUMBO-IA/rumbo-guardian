...
try {
  ...
  $profile = Join-Path $env:TEMP "rumbo-guardian-ledger-$port"
  if (Test-Path $profile) { Remove-Item -Recurse -Force $profile }  $browser = Start-Process -FilePath $brave -ArgumentList @("--remote-debugging-port=$port","--user-data-dir=$profile",'--no-first-run','--no-default-browser-check','about:blank') -PassThru
  Start-Sleep -Seconds 2
  $base = "http://127.0.0.1:$port"
  $version = Invoke-RestMethod "$base/json/version" -TimeoutSec 5
  Write-Output "BRAVE_LEDGER_READY $($version.Browser)"
  $previous = $env:GUARDIAN_CDP_URL
  $env:GUARDIAN_CDP_URL = $base
  Push-Location $project
  try { node .\tests\ledger-browser-integration.js }
