...
async function scan(){
  ...
  try{
    ...
    const [execution]=await chrome.scripting.executeScript({target:{tabId:tab.id},func:()=>({url:location.href,title:document.title||'',text:(document.body?.innerText||'').slice(0,7000),passwordFields:document.querySelectorAll('input[type="password"]').length,forms:document.forms.length,links:[...new Set([...document.links].map(a=>a.href).filter(Boolean))].slice(0,80)})});    const data=execution.result,lists=await getLists();
    ...
    if(data.forms>4){score=Math.min(100,score+5);reasons.push({points:5,title:'Múltiples formularios',detail:'La página contiene varios formularios.'});}
    const rankedLinks=Core.rankLinks(data.links,lists);const meta=Core.riskMeta(score);
    renderPage({score,...meta,reasons,domainStatus:base.domainStatus,rankedLinks},data);
