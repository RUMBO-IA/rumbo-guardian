...
RUMBO Guardian V1.0.0 is designed as a local-first, operator-controlled analysis layer.
...
V0.6.0 inspects only a bounded allowlist of common redirect-parameter names and only absolute HTTP(S) targets. It does not follow network redirects, fetch remote content, or claim that every external redirect is malicious. Same-site targets are not penalized as external redirects. V1.0.0 follows at most three recognized redirect hops, stops repeated routes and limits each URL to four recognized redirect targets.
